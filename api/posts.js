...
export const fetchPostHistory = async (postAddress, author) => {
	const nodeUrl = await getNodeUrl();
	const transactions = await makeRequest(`${nodeUrl}/transactions/confirmed?signerPublicKey=${author.publicKey}&recipientAddress=${postAddress}&type=16724&pageSize=500&pageNumber=1&order=desc&embedded=true`);
	const postMap = {};

	for (const transactionContainer of transactions.data) {
